...
@Form.create()
class BatchDisAuthorityModal extends PureComponent {
  state = {
    width: '100%',
...
// 获取选中的行
  ...
  render() {
    const { form, dispatch, submitting , rule: { data }, loading, BatchDisAuthorityVisible, handleBatchDisAuthorityVisible} = this.props;
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleBatchDisAuthorityVisible(false);
          form.resetFields();
          message.success('成功');
        }
      });
    ...
    return (
      <Modal
        title="批量分配权限"
        style={{ top: 20 }}
        visible={BatchDisAuthorityVisible}
        width="70%"
        maskClosable={false}
        onOk={validate}
        onCancel={onCancel}
...
}))(Form.create()(BatchDisAuthorityModal));
