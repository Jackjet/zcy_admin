...
class ReceiptPlanAddModal extends PureComponent {
  state = {
    width: '100%',
  };
  componentDidMount() {
  ...
  render() {
    const { form, dispatch, submitting, receiptPlanAddVisible, handleReceiptPlanAddVisible } = this.props;
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleReceiptPlanAddVisible(false);
        }
      });
    };
    ...
    return (
      <Modal
        title="收款计划基本信息新增"
        style={{ top: 20 }}
        visible={receiptPlanAddVisible}
        width="90%"
        maskClosable={false}
        onOk={validate}
        onCancel={() => handleReceiptPlanAddVisible()}
      >
        <div>
          <Card>
            <Form layout="inline">
...
}))(Form.create()(ReceiptPlanAddModal));
