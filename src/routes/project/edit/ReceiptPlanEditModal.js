...
class ReceiptPlanEditModal extends PureComponent {
  state = {
    width: '100%',
  };
  componentDidMount() {
  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleReceiptPlanEditVisible(false);
        }
      });
    };
    ...
    return (
      ...
      <Modal
        ...
        maskClosable={false}
        onOk={validate}
        onCancel={() => handleReceiptPlanEditVisible()}
      >
        <div>
          <Card>
            <Form layout="inline">
...
}))(Form.create()(ReceiptPlanEditModal));
