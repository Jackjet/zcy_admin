  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          form.resetFields();
          handleReceivablesViewVisible(false);
        }
      });
    };
    ...
    return (
      <Modal
        ...
        style={{ top: 150 }}
        // 对话框是否可见
        visible={receivablesViewVisible}
        width="60%"
        // 点击蒙层是否允许关闭
        maskClosable={false}
        onOk={validate}
        onCancel={() => handleReceivablesViewVisible()}
      >
        <div>
          <Card>
            <Form layout="inline">
                ...
                <Col span={12}>
                  <Form.Item {...formhz11} label={fieldLabels.receivablesType}>
                    {getFieldDecorator('receivablesType', {
                      rules: [{ required: false, message: '请选择收款类型' }],
                    })(
                      <Select disabled placeholder="请选择收款类型" style={{ width: 200 }}>
