...
@Form.create()
class CommissionSettingViewModal extends PureComponent {
  ...
  render() {
    const { form, dispatch, submitting , commissionSetViewVisible, handleCommissionSetViewVisible, rowInfo } = this.props;
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleCommissionSetViewVisible(false);
        }
      });
    };
    const onCancel = () => {
    ...
    return (
      ...
      <Modal
        title="提成比例系数查看"
        style={{ top: 20 }}
        visible={commissionSetViewVisible}
        width="30%"
        maskClosable={false}
        onOk={validate}
        onCancel={onCancel}
                ...
                <Col>
                  <Form.Item {...formhz11} label={fieldLabels.commissionType}>
                    {getFieldDecorator('commissionType', {
                      ...
                      <Select disabled onMouseEnter={this.handleCommissionSettingChange} placeholder="请输入提成比例类别" >
                        {commissionSettingOptionData}
                      </Select>
                    )}
                  </Form.Item>
                ...
                <Col>
                  <Form.Item {...formhz11} label={fieldLabels.remarks}>
                    {getFieldDecorator('remarks', {
                      ...
                      <TextArea disabled  placeholder="请输入备注" rows={4} />
                    )}
                  </Form.Item>
                </Col>
              </Row>
...
}))(Form.create()(CommissionSettingViewModal));
