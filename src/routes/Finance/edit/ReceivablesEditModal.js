...
class ReceivablesEditModal extends PureComponent {
  state = {
    width: '90%',
  };
  componentDidMount() {
  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          form.resetFields();
          handleReceivablesEditVisible(false);
        }
      });
    };
    ...
    return (
      <Modal
        ...
        style={{ top: 150 }}
        // 对话框是否可见
        visible={receivablesEditVisible}
        width="60%"
        // 点击蒙层是否允许关闭
        maskClosable={false}
        onOk={validate}
        onCancel={() => handleReceivablesEditVisible()}
      >
        <div>
          <Card>
            <Form layout="inline">
...
}))(Form.create()(ReceivablesEditModal));
