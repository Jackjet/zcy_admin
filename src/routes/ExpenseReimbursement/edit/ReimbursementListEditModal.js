...
class ReimbursementListEditModal extends PureComponent {
  state = {
    width: '100%',
    panelOptionData: [],
    industryOptionData: [],
  ...
  render() {
    const {
      ...
      handleReimbursementListEditModalVisible,
      rowInfo,
    } = this.props;
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleReimbursementListEditModalVisible(false);
        }
      });
    };
    const onCancel = () => {
    ...
    return (
      <Modal
        title="费用报销申请单信息查看"
        style={{ top: 20 }}
        visible={reimbursementListEditModalVisible}
        width="80%"
        maskClosable={false}
        onOk={validate}
        onCancel={onCancel}
...
}))(Form.create()(ReimbursementListEditModal));
