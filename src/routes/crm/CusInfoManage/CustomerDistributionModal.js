...
class CustomerDistributionModal extends PureComponent {
  state = {
    width: '100%',
    dataSource: [
      {
  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleCustomerDistributionVisible(false);
        }
      });
    };
    ...
    return (
      <Modal
        title="客户分配信息设置"
        style={{ top: 20 }}
        visible={customerDistributionVisible}
        width="80%"
        maskClosable={false}
        onOk={validate}
        onCancel={() => handleCustomerDistributionVisible()}
      >
        <div>
          <Card>
...
}))(Form.create()(CustomerDistributionModal));
