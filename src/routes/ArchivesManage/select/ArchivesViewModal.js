...
@Form.create()
class ArchivesViewModal extends PureComponent {
  state = {
    width: '90%',
    selectedRows: [],
    mockData: [],
  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleArchivesViewVisible(false);
        }
      });
    };
    ...
    return (
      <Modal
        ...
        style={{ top: 150 }}
        // 对话框是否可见
        visible={archivesViewVisible}
        width="60%"
        // 点击蒙层是否允许关闭
        maskClosable={false}
        onOk={validate}
        onCancel={() => handleArchivesViewVisible()}
      >
        <div>
          <Card>
