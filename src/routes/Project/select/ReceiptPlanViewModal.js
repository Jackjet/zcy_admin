...
class ReceiptPlanViewModal extends PureComponent {
  state = {
    width: '100%',
  };
  componentDidMount() {
  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          handleReceiptPlanViewVisible(false);
        }
      });
    };
    ...
    return (
      <Modal
        title="收款计划基本信息编辑"
        visible={receiptPlanViewVisible}
        width="90%"
        maskClosable={false}
        onOk={validate}
        onCancel={() => handleReceiptPlanViewVisible()}
      >
        <div>
          <Card>
            <Form layout="inline">
...
}))(Form.create()(ReceiptPlanViewModal));
